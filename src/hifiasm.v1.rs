...
pub fn read_seq_lens(path: &str) -> Vec<u32>{
	let mut buffer = [0u8; 44];
	let mut reader = BufReader::new(File::open(path).unwrap());

	reader.read_exact(&mut buffer).expect("Failed to parse the ec bin file");
	...
	let _total_name_length = u64::from_ne_bytes(buffer[36..].try_into().unwrap()) as usize;

	let mut N_sites = Vec::with_capacity(reads_count);
	for _ in 0..reads_count {
		reader.read_exact(&mut buffer[..8]).expect("Failed to parse the ec bin file");
	...
	// reader.seek(std::io::SeekFrom::Current(read_seperate as i64)).expect("Failed to seek the ec bin file");

	// let mut read_names = vec![0; total_name_length];
	// reader.read_exact(&mut read_names[..]).expect("Failed to parse the ec bin file");
...
pub fn iter_seq_bin(path: &str) -> impl Iterator<Item = Vec<u8>>{

	fn init_seqbits() -> Vec<Vec<u8>> {
		let bases = [b'A', b'C', b'G', b'T'];
		let mut seqbits = Vec::with_capacity(256);
		for i in 0..256{
			seqbits.push(vec![bases[(i >> 6) & 3], bases[(i >> 4) & 3], bases[(i >> 2) & 3], bases[i & 3]]);
		}
		seqbits
	}
	...
	read_lens.into_iter().map(|x| x as usize).enumerate().map(move |(i, len)| {
		buffer.clear();
...
type Ovl = (u32, u32, u32, u32, char, u32, u32, u32, u32, u32, u32);
pub fn iter_ovl_bin<'a>(path: &'a str, read_lens: &'a [u32]) -> impl Iterator<Item = Ovl> + 'a{
    let mut buffer = [0u8; 42];
    let mut length = 0;
    let mut reader = BufReader::new(File::open(path).unwrap());
    reader.read_exact(&mut buffer[..8]).expect("Failed to parse the ovl bin file");
    ...
    std::iter::from_fn(move || {
        if read_count >= 0 {
            while length == 0 {
                ...
                reader.read_exact(&mut buffer[..6]).expect("Failed to parse the ovl bin file");
                length = i32::from_ne_bytes(buffer[2..6].try_into().unwrap());
                read_count -= 1;
            }
            length -= 1;
            reader.read_exact(&mut buffer).expect("Failed to parse the ovl bin file");
            ...
            let (qn, qs) = ((qns >> 32) as u32, (qns & 0xFFFFFFFF) as u32);
            ...
            return Some((qn, ql, qs, qe - 1, std, tn, tl, ts, te - 1, mch, aln));
        }
        None
    })
}
...
//	for (i, seq) in iter_seq_bin(&path).enumerate() {
